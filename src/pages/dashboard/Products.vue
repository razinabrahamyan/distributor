...
import {truncateText, hasMore} from '@/utils/utils.js'
...
<template>
  <router-link to="cart" class="fixed right-10 bottom-10 bg-white p-2 border-red-500 border-2 rounded-md shadow-lg">
    <ShoppingCart class="text-red-500" stroke-width="2"/>
  </router-link>
  <div class="grid grid-cols-12 gap-4 pb-20">
    <div class="col-span-6 sm:col-span-4 md:col-span-6 lg:col-span-4 xl:col-span-3" v-for="item in items">
      <Card class="h-full flex flex-col">
        ...
        <CardContent class="!p-4 lg:!p-6 flex flex-col flex-1 justify-between">
          <CardTitle class="text-sm leading-4">{{truncateText(item)}} <button v-if="hasMore(item.name)" @click="expand(item)" :class="`underline ${item.expanded ? 'text-blue-400 font-normal' : 'font-semibold text-blue-500'}`">
            {{ item.expanded ? 'пок.меньше' : 'пок.больше' }}</button></CardTitle>
          <div class="flex justify-between mt-2">
            <div></div>
            <div>
              <button v-if="!cart.has(item)" @click="addToCart(item)" class="flex items-center rounded-full px-4 gap-2 bg-blue-500 p-1 text-xs text-white">Доб <ShoppingCart size="15"/></button>
